...
//			HttpPost post = new HttpPost(navigatorUrl+"/p8/logon.do");
    ...
    protected JsonResponse savePlugin() throws Exception {
        getLog().info("Saving plugin configuration : ");
        try {
            getLog().info(getJarToDeploy().getAbsolutePath());

            AbstractPlugin plugin = ICNUtils.loadFromJar(getJarToDeploy());
            plugin.setFilename(getRemoteFile().getPath());


            if (this.configuration != null) {
                getLog().info("Found configuration : " + configuration);
                plugin.setConfiguration(this.configuration);
            } else {
                getLog().info("No plugin configuration defined.");
            }
